...
```
/            キャラ一覧（アイコングリッド、自キャラ先頭ピン、検索、モードセレクタ+各行の勝/負ボタン＝ADR-0015）
/c/:slug     統一キャラページ。タブ（?tab=）:
               frames | punish | notes | record   …全キャラ共通（record=戦績、ADR-0015）
               own    | moves  | matches           …is_mainのみ追加（立ち回り/技メモ/試合）
/me          → /c/zero_suit_samus?tab=own へリダイレクト（廃止）
/search      横断検索（レーン分け維持）
/stats       全体戦績ダッシュボード（勝率時系列/モード別サマリ/連勝連敗/キャラ別ランキング＝ADR-0015）
```
...
## 戦績ブロック / `/stats` ダッシュボード（ADR-0015）

- **一覧**: 検索欄の下に「記録先」モードセレクタ（VIP/スマメイト/オフライン、端末に永続）。各行の右に**勝/負ボタン**（1タップ=1試合を現モードに記録、数秒間「取消」でundo）。行にはそのモードの `W-L 🔥連勝` を併記（狭幅では省略）。行のLink（詳細遷移）とボタンのネスト不正を避けるため hover背景・区切りは `<li>` 側に持つ
- **キャラ詳細の戦績ブロック**（旧recordタブ→キャラ名下の常設ブロックへ変更、詳細は docs/09）: 全キャラ共通（相手キャラ単位のため is_main 不問）。vsそのキャラの — モード別サマリ / 連勝・連敗（現在+最長）/ 勝率の時系列推移（累積）。モード選択+勝/負は常時、詳細は折りたたみ可
- **`/stats`**: 全体ダッシュボード。モードフィルタ（全体+3モード）と4グラフ — ①勝率の時系列推移（SVG polyline）②モード別サマリ（横バー3行）③連勝/連敗 ④キャラ別ランキング（勝率降順で得意/苦手）。ゲストにも表示（自分のローカル戦績を試せる）
- チャートはライブラリ非導入で手書きSVG/CSS（デザイントークン準拠、勝=赤/負=中立グレー、数値は font-frame tabular-nums）
- 詳細なデータモデル・RLS・プロバイダ分離は [docs/09_match-tracking.md](09_match-tracking.md) / [ADR-0015](adr/0015-match-tracking.md) を参照
