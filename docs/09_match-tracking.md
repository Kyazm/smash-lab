...
### キャラ詳細の戦績ブロック（旧「戦績」タブ）
当初はタブだったが廃止し、**キャラ名下の常設ブロック**（`CharacterStatsTab`）に変更（2026-07）。`is_main` 不問（対戦相手キャラ単位のため全キャラ共通）。「vsこのキャラ」の成績を表示する。モード選択+勝/負ボタンは常時表示、詳細（連勝・推移・モード別サマリ）は折りたたみ可（既定展開）。旧 `?tab=record` リンクは既定タブ（キャラ対メモ）へフォールバック。

タブ順序: フレーム表 / 確定反撃 / キャラ対メモ / 立ち回り / 技メモ / 試合
...
- ブラウザでの実データ検証: 一覧でのモード切替→記録→undo、キャラ詳細の戦績ブロック、`/stats`、ゲストがDB非書込であること
