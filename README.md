...
閲覧系はPWA対応（ホーム画面追加でオフラインでもフレームデータ/確反を参照可）。認証は単一オーナー。オーナーのメモ・AI整頓提案は個人のものとしてRLSでオーナー限定（notes/note_media/note_proposals は is_writer()）。ゲストは公開情報（フレームデータ・確反・ライブラリ記事）を閲覧でき、メモは自分のローカルサンドボックスに追加して機能を試せる（オーナーの実データは見えず・不変）。
