  ...
  return (
    <details open={defaultOpen} className="mt-4 rounded border border-border bg-surface-1/50 p-3 text-xs">
      ...
      <ul className="mt-2 space-y-1 text-ink-secondary">
        ...
        <li>
          ・ガーキャン空中攻撃は相手の身長・姿勢で当たらないことがある（例:
          勇者・ホムラ/ヒカリは低姿勢/姿勢で回避しうる）。フレーム上確定でも当たるかは要確認。相手別の空振りはキャラ対メモに記録推奨
        </li>
        <li>・on_shield は最終持続Fを密着ガードした場合の代表値。先端当て・持続当て・ワンパターン相殺で数F変動する</li>
        <li>
          ・<span className="text-warning">オンライン注意</span>:
          SPは遅延ベース通信のため実効+4〜6F程度の入力遅延が乗る。ここの確定はオフライン基準で、オンラインでは確定しない/猶予が減る反撃がある（反応依存の確反ほど影響大）
        </li>
